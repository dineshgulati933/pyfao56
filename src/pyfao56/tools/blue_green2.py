...
class BlueGreen2:
    """
    A modified water accounting model that partitions water balance components into blue (irrigation) and green (rainfall) water.
    ...
    def run(self):
        ...
        for idx, row in df.iterrows():
            ...
            Rain = row['Rain']
            Irrig = row['Irrig']
            S_prev = S if idx > self.startDate.strftime('%Y-%j') else 0.0 # previous day's available soil water (It will automatically will use previous S after first iteration so that will be prev day's S)
            S = row['TAW'] - row['mDr'] # current day's available soil water (mDr will be same as Dr if not assimilating VMC)
            E = row['E']
            T = row['T']

            # Logic to make sure S remains  non-zero and positive, we will use previous S if S is zero or negative
            if S <= 0:
                S = S_prev
            ...
            # Save sep and saw if non-zero to use if nex day's fractions turned to be zero
            if sep + saw > 0:
                last_sep = sep
                last_saw = saw
            ...
            if sep + saw == 0:
                ...
                # sep = 0.01
                # saw = 0.01 # This will set both sep and saw to small non-zero values, means equally partitioned which may not be accurate

            # This will nullify next if condition below since sep + saw will not be zero now but we will keep it for safety
